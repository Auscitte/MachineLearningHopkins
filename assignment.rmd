...
Cross-validation accuracy is quite high which might be an indicator of overfitting if the accuracy obtained for validation data has a sufficienly smaller value. We check it next.  

Now the resulting model is estimated.

In addition to cross-validation, we will validate the model on the *training* data set, which is used once only for evaluation purposes.   
...
Out-of-sample error provides an estimate of model accuracy. Later appears to be sufficiently high to accept the model. Now we will run prediction algorithm on the evaluation data supplied as part of the assignment.
...
## Conclusion
Using random forests learning algorithm we constructed a model which was able to predict whether the exercise was performed correctly on evaluation data with 100% accuracy. The fact that variable importance varies significantly suggests that a simpler model could be built.  
