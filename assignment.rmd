...
Due to the fact that computational resources available to us were limited the "random forest" learning algorithm provided a suitable compromise between accuracy (the next best algorithm gave us out-of-sample error 0.68 only) and running time. Parameters for the learning algorithm and resampling procedure were set based on the number of experiments.

We start by partitioning the data into training and testing sets. 
...
Next, the resampling method is chosen. In our case it is cross-validation with the number of folds equal to 3 which constitutes a sufficient tradeoff between speed and accuracy. 

```{r, cache = TRUE}
ctrl <- trainControl(method = "cv", number = 3, repeats = 1, verboseIter = TRUE)
```

The typical values for number of trees to grow are 10, 30, or 30; we select the medium one. Random forest does not require data prepossessing such as normalization of principal component analysis, therefore, we do not initialize *preProcess* parameter. Finally, *tuneLength* allows to specify how many different sets of parameters will be generated by caret. We set this value equal to the number of folds cross-validation uses.
...
Now we estimate the resulting model. 

```{r, cache = TRUE, warning=FALSE, message=FALSE}
prtrain <- predict(modFit, newdata = training)
cmtr <- confusionMatrix(prtrain, training$classe)
print(paste("In-sample error is equal to ", cmtr$overall[1]))
```

Redistribution error is quite high, thus, low out-of-sample error will be an indicator of overfitting.   

```{r, cache = TRUE, warning=FALSE, message=FALSE}
prtest <- predict(modFit, newdata = testing)
cmtst <- confusionMatrix(prtest, testing$classe)
print(paste("Out-of-sample error is equal to ", cmtst$overall[1]))
```

Out-of-sample error provides an estimate of model accuracy. Now we will run prediction algorithm on the validation data.
...
Now we will see what variables are actually used for outcome prediction.
```{r, cache = TRUE, echo = FALSE, warning=FALSE, message=FALSE}
varImp(modFit)
```

## Conclusion
Using random forests learning algorithm we were able to construct a model which was able to predict validation data with 100% accuracy. The fact that variable importance varies significantly suggests that a simpler model could be built.  
